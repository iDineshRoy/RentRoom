...
def search_view(request):
    ...
    if query is not None:
        ...
        # obj_list.extend(list(Product.objects.filter(qset_nep_and, status='Available').order_by('-id')))
        # obj_list.extend(list(Product.objects.filter(qset_nep_or, status='Available').order_by('-id')))
